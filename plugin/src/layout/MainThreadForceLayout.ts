...
export class MainThreadForceLayout implements LayoutEngine {
	...
	private comms: Int32Array = new Int32Array(0);
	...
	init(data: GraphData, positions: Float32Array, params: LayoutParams, initialAlpha = 1): void {
		...
		// forceSimulation starts its own internal timer on creation —— stop it at once and let the render loop drive it frame by frame instead
		this.sim = forceSimulation(this.simNodes, 3)
			...
			.force('community', forceCommunity(params.community, this.comms))
			.stop();
	...
	/** d3's default strength = 1/min(endpoint connections), with the multiplier on top (keeping the property that a hub is not pulled apart) */
	...
	updateParams(params: LayoutParams): void {
		...
		sim.force('community', forceCommunity(params.community, this.comms));
		this.reheat(0.5);
	}
