...
import { forceRadialCore, forceSpiral, forceCommunity } from './galaxyForces';

/**
 * The layout Worker (M3): d3-force-3d entirely off the main thread.
 * The protocol: init (coordinates, edges and degrees transferred in) → batched ticks (≤12ms per batch) →
 * the coordinates come back through a double-buffered transferable ping-pong (zero copy), returned once consumed.
 */
...
interface InitMsg {
	...
	comms: ArrayBuffer;   // Int32Array — the community index (-1 = none)
	params: LayoutParams;
	initialAlpha: number;
	bufA: ArrayBuffer;
	bufB: ArrayBuffer;
...
let comms: Int32Array = new Int32Array(0);
...
function applyParams(params: LayoutParams): void {
	...
	// A custom force has no clean .strength() setter → rebuild with the new strength (one O(n) initialize)
	sim.force('core', forceRadialCore(params.coreGravity, degrees));
	sim.force('spiral', forceSpiral(params.spiral));
	sim.force('community', forceCommunity(params.community, comms));
}

function schedule(): void {
	if (scheduled) return;
	scheduled = true;
	// The Worker scope uses self (there is no window); a macrotask yield lets the return messages be handled between batches
...
function run(): void {
	...
	if (!s || settled) return;
	const t0 = Date.now();
	// Batched ticks: at most 12ms per batch, with the main thread taking the latest frame at its own consumption rate
...
function post(): void {
	...
	if (!buf) return; // resend once the buffer is returned
...
ctx.onmessage = (e: MessageEvent) => {
	...
	switch (msg.type) {
		case 'init': {
			...
			comms = new Int32Array(msg.comms);
			...
			sim = forceSimulation(nodes, 3)
				...
				.force('community', forceCommunity(msg.params.community, comms))
				.stop();
		...
		case 'buffer':
			...
			post(); // settling's last frame may be waiting on a buffer
			break;
	}
};
