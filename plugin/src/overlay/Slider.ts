...
/**
 * A Lightroom-style parameter slider (G1 feedback: the default centred + the limits made visible).
 * - The default value is always at the track's geometric centre: piecewise linear over [min,default] on the left half and [default,max] on the right
 * - The centre notch = the default position; the fill bar is drawn from the notch to the handle (the direction and size of the deviation are visible at a glance)
 * - The min/max limits sit permanently at both ends of the track; a double click returns to the default
 */
export class Slider {
	...
	/** Position (0..1) → value: the default is anchored at 0.5 */
