...
/**
 * `{var|singular|plural}` — the word agrees with the count in `var`.
 * Written inline so each language keeps whole sentences (Spanish also swaps its verb).
 * ponytail: 1 vs. rest only. A language with few/many (ru, pl, ar) needs Intl.PluralRules instead.
 */
const PLURAL = /\{(\w+)\|([^{}|]*)\|([^{}|]*)\}/g;
...
export function t(key: keyof Dict, vars?: Record<string, string | number>): string {
	...
	if (vars) {
		s = s.replace(PLURAL, (_m, k: string, one: string, other: string) => (Number(vars[k]) === 1 ? one : other));
