...
> A fork of [Galaxy View](https://github.com/longwind1984/galaxy-view) by Rick,
> extended for Kallimachos. See **Install** below.
...
> [longwind1984/galaxy-view](https://github.com/longwind1984/galaxy-view) by Rick —
> that is what the community store ships. This fork adds the Kallimachos entity/relation
> graph (`kal-graph.json`) and is **not** in the store.

**From source** (this repo):

```bash
cd plugin && npm install && npm run build
cp dist/{main.js,manifest.json,styles.css} <vault>/.obsidian/plugins/kal-galaxy/
```

then enable it in Settings → Community plugins.

> It installs into `kal-galaxy/`, deliberately **not** upstream's `galaxy-view/`, so
> you can keep Rick's store version installed alongside this fork. A selftest in
> `src/export_kal_graph.py` asserts the manifest id and every hard-coded install
> path still agree — they are spread over seven files, and when they drift the view
