...
/**
 * `valueToPos`'s two degenerate guards —— `defaultValue === min` and `max === defaultValue`.
 *
 * They look marginal and **a shipped preset really produces them.**  `minimal` sets
 * look.{linkCurve,twinkle} = 0 and those sliders all have min 0
 * (stylePresets.ts:70-73).  That is, one preset sends 6 sliders down this branch.
 *
 * Without the guard, `(v - min) / (d - min)` = 0/0 = NaN, and refresh() writes
 * `style.left = "NaN%"` —— the handle disappears with no error.
 * (found by mutation testing on 2026-08-21, where this branch alone survived)
 */
describe('Slider — the degenerate spec with the default at either end', () => {
	const pos = (t: ReturnType<typeof makeSlider>) =>
		t.slider === undefined ? '' : (t.track.find('gx-slider-thumb').style.left ?? '');

	it('the position stays finite even with defaultValue === min (the minimal preset)', () => {
		const t = makeSlider({ min: 0, max: 1, defaultValue: 0, step: 0.05 });
		const left = pos(t);
	...
	it('the position stays finite even with defaultValue === max', () => {
		const t = makeSlider({ min: 0, max: 1, defaultValue: 1, step: 0.05 });
		const left = pos(t);
		expect(left).not.toContain('NaN');
		expect(Number.parseFloat(left)).toBeLessThanOrEqual(100);
	});
	...
	/**
	 * The `max === d` guard is not reachable on the normal path —— `applyValue` clamps, so
	 * `v > d(=max)` cannot happen.  But `refresh()` reads `spec.get()` **as-is**, so a value
	 * outside the range left in the settings file (an older version having used a wider max)
	 * arrives here.  Without the guard that divides by zero to Infinity and gives
	 * `style.left = "Infinity%"`.
	 */
	it('the position stays finite when a saved value exceeds max (d === max)', () => {
		const t = makeSlider({ min: 0, max: 1, defaultValue: 1, step: 0.05 });
		t.write(5);                       // an out-of-range value left by an older version
		t.slider.refresh();
		const left = pos(t);
		expect(left).not.toContain('Infinity');
		expect(left).not.toContain('NaN');
		expect(Number.isFinite(Number.parseFloat(left))).toBe(true);
	});

	it('the handle still moves right as the value grows, even on a degenerate spec', () => {
		const t = makeSlider({ min: 0, max: 1, defaultValue: 0, step: 0.05 });
		const at0 = Number.parseFloat(pos(t));
		t.write(0.8);
		t.slider.refresh();
		const at08 = Number.parseFloat(pos(t));
		expect(at08).toBeGreaterThan(at0);
		expect(Number.isFinite(at08)).toBe(true);
	});
});
