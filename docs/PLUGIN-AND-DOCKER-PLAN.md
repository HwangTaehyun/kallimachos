...
| 4 | the `no_llm` gate ships effectively disabled | 0 documents in the vault have `no_llm: true` → `llm_gate` always passes (`schema_v3.py:581`) | **§6 step 11** |
| 5 | `SKIP` is coupled to this machine's folder name | substring matching (`schema_v3.py:182` — `SKIP_ANY` is at `:89`). Cloning as `kal/` gets it indexed and sent | **§6 step 12** |
